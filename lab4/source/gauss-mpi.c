...
int main(int argc, char **argv)
{
    ...
    for (t = 0; t < T; t++) {
		...
		/* Communicate */
        MPI_Request requests_before_comp[6], requests_after_comp[2];
        int req_before_comp_cnt = 0, req_after_comp_cnt = 0;
        ...
        // North - Before computation
        MPI_Isend(&u_previous[1][1], 1, row, north, 0, MPI_COMM_WORLD, &requests_before_comp[req_before_comp_cnt++]);
        MPI_Irecv(&u_current[0][1], 1, row, north, 0, MPI_COMM_WORLD, &requests_before_comp[req_before_comp_cnt++]);

        // South - Before computation
        MPI_Irecv(&u_previous[local[0]+1][1], 1, row, south, 0, MPI_COMM_WORLD, &requests_before_comp[req_before_comp_cnt++]);

        // East - Before computation
        MPI_Irecv(&u_previous[1][local[1]+1], 1, col, east, 0, MPI_COMM_WORLD, &requests_before_comp[req_before_comp_cnt++]);

        // West - Before computation
        MPI_Isend(&u_previous[1][1], 1, col, west, 0, MPI_COMM_WORLD, &requests_before_comp[req_before_comp_cnt++]);
        MPI_Irecv(&u_current[1][0], 1, col, west, 0, MPI_COMM_WORLD, &requests_before_comp[req_before_comp_cnt++]);

        MPI_Waitall(req_before_comp_cnt, requests_before_comp, MPI_STATUSES_IGNORE);
        ...
        for (i = i_min; i <= i_max; i++)
            for (j = j_min; j <= j_max; j++)
                u_current[i][j] = u_previous[i][j] +
				    (u_current[i-1][j] + u_previous[i+1][j] + u_current[i][j-1] + u_previous[i][j+1] - 4*u_previous[i][j]) * omega/4.0;
        ...
        // South - After computation
        MPI_Isend(&u_current[local[0]][1], 1, row, south, 0, MPI_COMM_WORLD, &requests_after_comp[req_after_comp_cnt++]);

        // East - After computation
        MPI_Isend(&u_current[1][local[1]], 1, col, east, 0, MPI_COMM_WORLD, &requests_after_comp[req_after_comp_cnt++]);

        MPI_Waitall(req_after_comp_cnt, requests_after_comp, MPI_STATUSES_IGNORE);
