...
int main(int argc, char **argv)
{
    ...
    for (t = 0; t < T; t++) {
			...
			for (j = j_min; j <= j_max; j++)
				if ((i + j) % 2 == 0)
					u_current[i][j] = u_previous[i][j] +
						(omega/4.0) * (u_previous[i-1][j] + u_previous[i+1][j] + u_previous[i][j-1] + u_previous[i][j+1] - 4*u_previous[i][j]);
        ...
        // North
        MPI_Isend(&u_current[1][1], 1, row, north, 0, MPI_COMM_WORLD, &requests[req_cnt++]);
        MPI_Irecv(&u_current[0][1], 1, row, north, 0, MPI_COMM_WORLD, &requests[req_cnt++]);

        // South
        MPI_Isend(&u_current[local[0]][1], 1, row, south, 0, MPI_COMM_WORLD, &requests[req_cnt++]);
        MPI_Irecv(&u_current[local[0]+1][1], 1, row, south, 0, MPI_COMM_WORLD, &requests[req_cnt++]);

        // East
        MPI_Isend(&u_current[1][local[1]], 1, col, east, 0, MPI_COMM_WORLD, &requests[req_cnt++]);
        MPI_Irecv(&u_current[1][local[1]+1], 1, col, east, 0, MPI_COMM_WORLD, &requests[req_cnt++]);

        // West
        MPI_Isend(&u_current[1][1], 1, col, west, 0, MPI_COMM_WORLD, &requests[req_cnt++]);
        MPI_Irecv(&u_current[1][0], 1, col, west, 0, MPI_COMM_WORLD, &requests[req_cnt++]);
        ...
        for (i = i_min; i <= i_max; i++)
			for (j = j_min; j <= j_max; j++)
				if ((i + j) % 2 == 1)
					u_current[i][j] = u_previous[i][j] +
						(omega/4.0) * (u_current[i-1][j] + u_current[i+1][j] + u_current[i][j-1] + u_current[i][j+1] - 4*u_previous[i][j]);
