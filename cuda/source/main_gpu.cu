...
int main(int argc, char **argv)
{
    ...
    // NOTE: We create a copy of clusters, so check_repeated_clusters() doesn't modify
    // the original array, and we have consistent results among implementations.
    float* clustersCopy = (float*)  malloc(numClusters * numCoords * sizeof(float));
    memcpy(clustersCopy, clusters, numClusters*numCoords*sizeof(float));
        
    // check initial cluster centers for repetition 
    if (check_repeated_clusters(numClusters, numCoords, clustersCopy) == 0) {
