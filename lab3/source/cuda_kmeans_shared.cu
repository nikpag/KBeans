                    ...
                    int    clusterId)
{
    ...
    for (i = 0; i < numCoords; i++) {
        float a = objects[i * numObjs + objectId];
        float b = clusters[i * numClusters + clusterId];
                          ...
                          float *devdelta)
{
    ...
    min_dist = euclid_dist_2_transpose(
        numCoords,
        numObjs,
        numClusters,
        deviceObjects,
						...
						int blockSize)
{
    ...
    double start, gpu_time = 0.0, transfer_time = 0.0, cpu_time = 0.0;
    ...
    /* DONE: Update clusters using dimClusters. Be careful of layout!!! clusters[numClusters][numCoords] vs dimClusters[numCoords][numClusters] */
	for(i = 0; i < numCoords; i++)
        for(j = 0; j < numClusters; j++)
    ...
    printf("analysis: gpu = %lf ms, transfer = %lf ms, cpu = %lf ms\n\n", 1000*gpu_time, 1000*transfer_time, 1000*cpu_time);
